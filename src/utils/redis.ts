...
import { parseJSON } from "../middleware/helpers";
...
export class CustomRedis {
  ...
  async set(key: string, data: any): Promise<any> {
    ...
    if (typeof data !== "number" || typeof data !== "string") data = JSON.stringify(data);
  ...
  async getCachedUser(id: string, throwError = true): Promise<any> {
    ...
    if (!user && throwError) throw new UnauthenticatedError("Kindly login, user not found");
    return parseJSON(user);
  }

  async cacheUser(user: any): Promise<void> {
    await Promise.all([this.set(user.tokenRef, user), this.set(`${user.id}-token`, user)]);
  }
